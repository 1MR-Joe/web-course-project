...
export function showProductInCart(data, card, container) {
    data.forEach(item => {
        // console.log(item);
        // duplicate card
        let cardCopy = card.cloneNode(true);
        ...
        cardCopy.querySelector('.product-img-tag').src = `${new URL(item.image, window.location.origin)}`;
        cardCopy.querySelector('.product-name').textContent = item.name;
